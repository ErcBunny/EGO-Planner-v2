...
Eigen::Matrix3d quaternion_to_R(const Eigen::Vector4d& q)
{
    Eigen::Vector4d nq = q.normalized();
...
Eigen::Vector4d R_to_quaternion(const Eigen::Matrix3d& R)
{
    Eigen::Vector4d q;
    double  tr = R(0,0) + R(1,1) + R(2,2);
    if (tr > 0)
...
Eigen::Vector<double, 6> pose_update(const Eigen::Vector<double, 6>& X1, const Eigen::Vector<double, 6>& X2)
{
    ...
    ypr1 = X1.segment(3, 3);
    ypr2 = X2.segment(3, 3);
    Eigen::Matrix3d R1 = ypr_to_R(ypr1);
    Eigen::Matrix3d R2 = ypr_to_R(ypr2);
    Eigen::Matrix3d R3 = R1 * R2;
    ypr3 = R_to_ypr(R3);

    Eigen::Vector3d X1xyz, X2xyz, X3xyz;
    X1xyz = X1.segment(0, 3);
...
Eigen::Vector<double, 6> pose_inverse(const Eigen::Vector<double, 6>& X)
{
    ...
    Eigen::Vector3d XIxyz = R.transpose() * X.segment(0, 3);
    Eigen::Vector3d XIypr;
    double XIy = atan2(o(0), n(0));
    double XIp = atan2(-a(0), n(0)*cos(XIy)+o(0)*sin(XIy));
    double XIr = atan2(n(2)*sin(XIy)-o(2)*cos(XIy), -n(1)*sin(XIy)+o(1)*cos(XIy));
    XIypr << XIy, XIp, XIr;
